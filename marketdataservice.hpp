...
public:
  ...
  const vector<Order>& GetOfferStack() const;
    ...
    void bidExpand(Order bid){
        auto i = bidStack.begin();
        for(;i!=bidStack.end();i++){
            if((*i).GetPrice()>=bid.GetPrice())
                continue;
            else
                break;
        }
        bidStack.insert(i, bid);
    }
    
    void offerExpand(Order offer){
        auto i = offerStack.begin();
        for(;i!=offerStack.end();i++){
            if((*i).GetPrice()<=offer.GetPrice())
                continue;
            else
                break;
        }
        offerStack.insert(i, offer);
    }
    
    void expand(OrderBook<T>& data){
        for(auto i = data.bidStack.begin(); i!=data.bidStack.end(); i++)
            bidExpand(*i);
        for(auto i = data.offerStack.begin(); i!=data.offerStack.end(); i++)
            offerExpand(*i);
    }
private:
  T product;
